...
def set_shear_values(left, right):
    ...
    # scene = context.scene
    action = bpy.data.actions.get('curvemotion')
    curves = utils.action_fcurves(action)
...
def shear(self, direction):
    ...
    shear_curves = utils.action_fcurves(shear_action)
...
def set_min_max_values(self, context):
    self.curvemotion = context.scene.curvemotion
    tool = self.curvemotion.tool
...
def to_execute(self, context, function, *args):
    ...
    for obj_name in global_objs:
        ...
        for action_type in global_actions:
            ...
            self.fcurves = utils.action_fcurves(action)
...
def reset_original():
    ...
    for obj_name in global_objs:
        ...
        for action_type in global_actions:
            ...
            for fcurve_i in global_fcurves:
                fcurve = utils.action_fcurves(action)[fcurve_i]
        ...
        for action_data in actions:
            ...
            for fcurve_index, fcurve in fcurves.items():
                ...
                for key_index, key in keys.items():
                    ...
                    # if curvemotion.tool.keys_under_cursor:
