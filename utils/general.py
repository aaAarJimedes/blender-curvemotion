...
def set_curvemotion_action():
    """Creates an "action" called 'curvemotion'"""

    action = bpy.data.actions.get('curvemotion')

    if action is None:
        return bpy.data.actions.new('curvemotion')
    else:
        return bpy.data.actions.get('curvemotion')
...
def add_marker(name, side, frame=0, overwrite=True):
    ...
    use_markers = curvemotion.tool.use_markers
