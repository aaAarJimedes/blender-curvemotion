...
def new(action_group_name, keys_to_add, key_interp='AUTO_CLAMPED', color=(1, 1, 1)):
    """Adds and fcuve in the 'curvemotion' action"""

    action = utils.set_curvemotion_action()

    fcurves = utils.action_fcurves(action)
    if not hasattr(action, 'layers'):
        blends_curve = fcurves.new(
            data_path=group_name, index=0, action_group=action_group_name)
    else:
        # Blender 4.3+: fcurves live in an ActionChannelbag and the
        # keyword used by the fcurves collection is group_name.
        blends_curve = fcurves.new(
            data_path=group_name, index=0, group_name=action_group_name)
...
def create_path(context, fcurves):
    curve_obj = add_curve3d(context, "curvemotion_path")
...
def get_all_fcurves(obj):
    ...
    trans_fcurves = utils.action_fcurves(trans_action)
    if trans_fcurves:
    ...
    if obj.type != 'ARMATURE':
        sk_data = getattr(obj.data, 'shape_keys', None)
        sk_anim = getattr(sk_data, 'animation_data', None)
        shapes_action = getattr(sk_anim, 'action', None)
        shapes_fcurves = utils.action_fcurves(shapes_action)
        if shapes_fcurves:
...
def remove_helpers(objects):
    ...
    for obj in objects:
        ...
        for fcurve in utils.action_fcurves(action):     # delete the first of the clones left
            if getattr(fcurve.group, 'name', None) == group_name:
                utils.action_fcurves(action).remove(fcurve)
...
def duplicate(fcurve, selected_keys=True, before='NONE', after='NONE', lock=False):
    ...
    if selected_keys:
        selected_keys = [
            (i, k) for i, k in fcurve.keyframe_points.items()
    ...
    if not hasattr(action, 'layers'):
        dup = fcurves.new(data_path=clone_name, index=index, action_group=group_name)
    else:
        dup = fcurves.new(data_path=clone_name, index=index, group_name=group_name)
    ...
    groups = utils.action_groups(action)
...
def duplicate_from_data(fcurves, global_fcurve, new_data_path, before='NONE', after='NONE', lock=False):
    ...
    if not hasattr(action, 'layers'):
        ...
        dup = fcurves.new(data_path=new_data_path, index=index, group_name=group_name)
...
def remove_clone(objects):
    ...
    for obj in objects:
        ...
        curvemotion = bpy.context.scene.curvemotion
        aclones = curvemotion.clone_data.clones
        clones_n = len(aclones)
        fcurves = utils.action_fcurves(action)
        blender_n = len(fcurves) - clones_n

        for n in range(clones_n):
            maybe_clone = fcurves[blender_n]     # delete the first of the clones left
...
def move_clone(objects):
    ...
    for obj in objects:
        ...
        curvemotion = bpy.context.scene.curvemotion
        aclone_data = curvemotion.clone_data
    ...
    if obj.type == 'ARMATURE':
        ...
        if check_ui:
            ...
            if context.area.type == 'VIEW_3D':
                ...
                if only_selected and not pose_bone.select:
                    return False
